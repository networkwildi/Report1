...
mandate_2000 <- mand_addr3 %>% 
  select(assembly, mandate_start_year_x, party) %>% 
    group_by(assembly) %>% 
      filter(mandate_start_year_x == 2000, party != "", assembly != "") %>% 
        count(assembly, party) %>% 
          arrange(desc(n)) %>% 
            mutate(lab_prop = prop.table(n)*100)



myTable = tableGrob(mandate_2000)
  myPlot = ggplot(mandate_2000, aes(x = party, y = n))+
    geom_point()
...
pai_plot = ggplot(mandate_2000, aes(x = "", y = lab_prop, fill=party))+
  geom_bar(width = 1, stat= "identity" ,color = "white")+
    coord_polar("y", start=0)+
      geom_text(aes(y = lab_prop, label = ""), color = "white")+
...
# Funktioniert !!
age_try <- mand_addr3 %>% 
  select(assembly,gender, party, mandate_start_year_x,mandate_end_year_x,year_of_birth, year_of_death) %>%
  filter(year_of_birth != "", year_of_death != "") %>% 
    filter(gender !="") %>%
      filter(year_of_birth != year_of_death) %>% 
        filter(party !="") %>%
...
party_list <- as_tibble(age_try) %>% 
    select(assembly, mandate_start_year_x, mandate_end_year_x ,party) %>% 
      mutate(mandate_start_year_x = as.numeric(mandate_start_year_x)) %>% 
        mutate(mandate_end_year_x = as.numeric(mandate_end_year_x)) %>% 
          mutate(party = as.character(party)) %>% 
            filter(party %in% c("SP", "SVP", "FDP", "APS","GP", "POCH","EVP", "CVP", "LdU","BGB","SVP/BGB")) %>% 
              group_by(assembly) %>% 
                count(party) %>% 
                   mutate(party_prop = prop.table(n)*100)

##################

party_list_all <- as_tibble(age_try) %>% 
    select(assembly, mandate_start_year_x, mandate_end_year_x ,party) %>% 
    mutate(party = as.character(party)) %>% 
      filter(party %in% c("SP", "SVP", "FDP", "APS","GP", "POCH","EVP", "CVP", "LdU","BGB","SVP/BGB")) %>% 
        mutate(mandate_start_year_x = as.numeric(mandate_start_year_x)) %>% 
          mutate(mandate_end_year_x = as.numeric(mandate_end_year_x)) %>% 
            group_by(assembly, party, mandate_start_year_x, mandate_end_year_x) %>% 
              count(party)
...
party_list_all %>%                  
ggplot(aes(x=mandate_start_year_x, y=assembly, fill(party)))+
  geom_line()+
    geom_point(size= 2,aes(color=party))+
  facet_grid(vars(assembly, party, n))
...
year_of_death <- mand_addr3 %>% 
  select(assembly,gender, party, title, year_of_birth, year_of_death) %>%
    filter(year_of_birth != "", year_of_death != "") %>% 
      filter(year_of_birth != year_of_death) %>% 
        mutate(year_of_birth = as.numeric(year_of_birth)) %>% 
          mutate(year_of_death = as.numeric(year_of_death)) %>% 
            mutate(age = (year_of_death - year_of_birth)) %>% 
              filter(age >=20 & age <=110) %>%  
                group_by(assembly)


avarage_live_span <-ave(year_of_death$age) %>% 
  round(2)
  as_factor(avarage_live_span)
avarage_live <-  unique(avarage_live_span)

live_span_result <- paste("The average live span ist",avarage_live)


year_of_death_by_title <- mand_addr3 %>% 
  select(title, year_of_birth, year_of_death) %>%
  filter(year_of_birth != "", year_of_death != "") %>% 
    filter(title !="") %>%
      filter(year_of_birth != year_of_death) %>% 
        mutate(year_of_birth = as.numeric(year_of_birth)) %>% 
...
avarage_live_span_by_title <- ave(year_of_death_by_title$age) %>% 
  round(2)
  as_factor(avarage_live_span_by_title)
    avarage_live_by_title <- unique(avarage_live_span_by_title)

        title_live_span <-  paste("The avarage live span by title is", avarage_live_by_title)
                    
  
avarage_both <- c(live_span_result, title_live_span)
avarage_both
...
most_mandates <- mand_addr3 %>% 
  select(lastname, firstname, year_of_birth, year_of_death, mandate_start_year_x, mandate_end_year_x, party,person_id) %>%
    filter(year_of_birth != "", year_of_death != "") %>% 
      filter(party != "", firstname != "", lastname != "") %>% 
        filter(party %in% c("SP", "SVP", "FDP", "APS","GP", "POCH","EVP", "CVP", "LdU","BGB","SVP/BGB")) %>% 
          mutate(year_of_birth = as.numeric(year_of_birth)) %>% 
            mutate(year_of_death = as.numeric(year_of_death)) %>% 
              mutate(age = (year_of_death - year_of_birth)) %>% 
                filter(age >=20 & age <=80) %>%  
                  ...
                  mutate(lastname = as.character(lastname), firstname = as.character(firstname), party = as.character(party))%>% 
      mutate(mandate_start_year_x = as.numeric(mandate_start_year_x), mandate_end_year_x = as.numeric(mandate_end_year_x))%>%
  distinct(person_id, mandate_start_year_x, mandate_end_year_x, .keep_all = TRUE) %>% 
    group_by(person_id, lastname, firstname,party) %>% 
                      unique() %>% 
                         ungroup()

most_mandates_top_1 <- most_mandates %>% 
  count(lastname, firstname,party, person_id) %>% 
  arrange(desc(n)) %>% 
  ungroup() %>% 
  head(10)
...
ggplot(data=most_mandates_top_1,aes(x=lastname, y=n,fill(party)))+
  geom_bar(stat = "identity", show.legend = TRUE, inherit.aes = TRUE,col= "darkblue")+
    coord_flip()+
        labs(title = "Politicians with the most mandates", x="Person", y="Mandates")
```
            ...
            mutate(year_of_death = as.numeric(year_of_death)) %>% 
              mutate(age = (year_of_death - year_of_birth)) %>% 
                filter(age >=20 & age <=80) %>%  
                  ...
                  mutate(lastname = as.character(lastname), firstname = as.character(firstname), party = as.character(party))%>% 
      mutate(mandate_start_year_x = as.numeric(mandate_start_year_x), mandate_end_year_x = as.numeric(mandate_end_year_x))%>%
  distinct(person_id, mandate_start_year_x, mandate_end_year_x,party, .keep_all = TRUE) %>% 
    group_by(person_id, lastname, firstname,party)

   double_mandates_top_10 <- double_mandates %>% 
    distinct(person_id, lastname, firstname, party, .keep_all = TRUE) %>% 
     count(lastname, firstname, person_id) %>%  
      group_by(lastname, firstname, person_id) %>% 
        arrange(desc(n)) %>% 
          ungroup()
   
             paste(" There ar no double mandates ")
```
...
# Do some politicians have multiple mandates at the same time?
 ...
 different_parties <- mand_addr3 %>% 
  select(lastname, firstname, mandate_start_year_x, mandate_end_year_x, party,person_id) %>%
    # filter(year_of_birth != "", year_of_death != "") %>% 
      filter(party != "", firstname != "", lastname != "") %>% 
        filter(party %in% c("SP", "SVP", "FDP", "APS","GP", "POCH","EVP", "CVP", "LdU","BGB","SVP/BGB")) %>% 
    #       mutate(year_of_birth = as.numeric(year_of_birth)) %>% 
...
are_there_affiliation_changes <- different_parties %>% 
     distinct(person_id, lastname, firstname, party, .keep_all = TRUE) %>% 
     count(lastname, firstname, person_id) %>%  
      group_by(lastname, firstname, person_id) %>% 
        arrange(desc(n)) %>% 
          ungroup()
             ...
             paste(" There were no persons affiliated to differen parties ")

different_parties_test <- mand_addr3 %>% 
  select(person_id, lastname, firstname, party) %>% 
      filter(party != "", firstname != "", lastname != "") %>% 
        filter(party %in% c("SP", "SVP", "FDP", "APS","GP", "POCH","EVP", "CVP", "LdU","BGB","SVP/BGB")) %>% 
          distinct(person_id, lastname, firstname, party) %>% 
        #    filter_if()           # filter wenn allesperson id = aber party ungleich
  count(person_id, lastname, firstname) %>% 
             arrange(desc(n)) %>% 
              filter(n != 1) %>% 
                   group_by(person_id)
