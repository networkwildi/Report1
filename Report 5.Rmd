...
tom_cruise <- search_person("54d37afd6cfdb6d71170db511bcf51b0", "tom cruise", include_adult = NA) %>%   as.data.frame() %>%  # search for TC
select(results.name, results.id)   # select name and id

will_ferrel <- search_person("54d37afd6cfdb6d71170db511bcf51b0", "will ferrel", page = 1, include_adult = NA) %>%    # search for WF
  as.data.frame() %>%    # transform to DF
    select(results.name, results.id)   # select name and id 

liam_neeson <- search_person("54d37afd6cfdb6d71170db511bcf51b0", "liam neeson", page = 1, include_adult = NA) %>%    #  search for LN
  as.data.frame() %>%   # transform to DF
    select(results.name, results.id)  # select name and id
...
mr_list <-most_revenue_parsed[["results"]]
